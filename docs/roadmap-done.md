...
[DONE 2026-07-26] fix: 「今日はやらない」（isDeferred）タスクがインボックスの一覧では隠れているのに、空き時間タップの配置候補（FreeSlotModal）とモバイルタブバーのインボックス件数バッジ（AppShellのinboxCount）には含まれたままだった矛盾を修正（ROADMAPが空だったため、コードベース調査で発見）。両方に`!t.isDeferred`を追加し、FreeSlotModalにはInboxPanelと同じ優先度→所要時間ソートも追加
[DONE 2026-07-26] feat: モバイルでインボックス/タイムライン間を左右スワイプで切り替えられるように（ユーザー依頼）。AppShellのモバイルコンテナにpointerイベントで水平スワイプを検出し、しきい値を超えたらactiveTabを切替。InboxTaskCardのスワイプ完了操作やTaskBlock/InboxTaskCardのドラッグ配置と競合しないよう、`data-task-card`/`data-task-id`を持つ要素・ボタン・入力欄上で始まったジェスチャーは除外。tailwind.config.tsに独自のslide-in-right/leftキーフレームを追加（既存のanimate-in系クラスはtailwindcss-animateプラグイン未導入のためno-opだったので、この機能用には確実に動く方式を新設）
